...
File name: type_m_g.h
Description: 群体攻击魔法公共方法
Author: xuefeng
Version: v1.0
Date: 2019-03-12
History:
...
int do_effect(object me, object who)
{
    ...
    me->start_busy(1 + random(spell_info["range"] - 6));
    me->add_attr("mp", -spell_info["mp"]);
    if(attack_type == TYPE_C)
    {
        msg = "$ME吟唱了" + chinese(spell_info["name"]) + "，爆发！效果大幅增加！";
        msg("warning", msg, me);
        msg = "对$YOU造成 " + damage + " 点爆击伤害。";
    } else {
        msg = "$ME吟唱了" + chinese(spell_info["name"]) + "。";
        msg("danger", msg, me);
        msg = "对$YOU造成 " + damage + " 点伤害。";
        }

    foreach(object ob in obs)
    {
        if (objectp(ob) && (member_array(ob->query("uuid"), ({46, 89, 153, 214, 229})) < 0))
        {
